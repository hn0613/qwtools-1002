 ...
 *
 * Central data definitions for all reader, transform, and writer component
 * types available in the JSON/YAML Creator. Each entry maps a class name
 * (used in the generated config) to a human-readable display name and the
 * list of kwarg parameter names the component accepts.
 */
...
/**
 * Look up the registry object for a given section type.
 * @param {'readers'|'transforms'|'writers'} type
 * @returns {Object} the registry map for that section
 */
...
/**
 * Reverse lookup: find the class name from a display name within a registry.
 * @param {string} displayName
 * @param {Object} registry - one of READERS, TRANSFORMS, WRITERS
 * @returns {string|null} the class name, or null if not found
 */
export function displayNameToClass(displayName, registry) {
  const found = Object.keys(registry).find(
    (k) => registry[k].displayName === displayName
  );
  return found || null;
}
